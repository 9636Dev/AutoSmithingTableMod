...
public class AutoSmithingTableScreen extends AbstractContainerScreen<AutoSmithingContainer> {
    ...
    public void render(@NotNull PoseStack stack, int mx, int my, float pPartialTick) {
        ...
        if (isIn(mx, my, i + ENERGY_BAR_ONS_LEFT,j + ENERGY_BAR_ONS_TOP,
            ...
            renderComponentTooltip(stack, List.of(Component.literal((this.menu.data.get(0) << 16 | Short.toUnsignedInt((short)menu.data.get(1))) + "/" + AutoSmithingTableConfig.COMMON.maxEnergyStored.get())), mx, my);
        }
    ...
    protected void renderBg(@NotNull PoseStack stack, float pPartialTick, int pMouseX, int pMouseY) {
        ...
        int mappedY = mapNum(menu.data.get(0) << 16 | Short.toUnsignedInt((short) menu.data.get(1)), AutoSmithingTableConfig.COMMON.maxEnergyStored.get(), ENERGY_BAR_HEIGHT);
